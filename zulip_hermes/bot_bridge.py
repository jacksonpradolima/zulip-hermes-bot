...
ZULIP_ALLOWED_USERS = {
    email.strip().lower() for email in os.getenv("ZULIP_ALLOWED_USERS", "").split(",") if email.strip()
}

ZULIP_REQUIRE_MENTION = os.getenv("ZULIP_REQUIRE_MENTION", "true").lower() == "true"
ZULIP_FREE_RESPONSE_STREAMS = {
    value.strip().lower() for value in os.getenv("ZULIP_FREE_RESPONSE_STREAMS", "").split(",") if value.strip()
}
...
def require_env() -> None:
    ...
    if not ZULIP_ALLOWED_USERS:
        raise SystemExit("ZULIP_ALLOWED_USERS is empty. Add your Zulip user email to .env.")
...
def stream_requires_mention(stream_name: str, stream_id: int | str | None = None) -> bool:
    ...
    return not (stream_key in ZULIP_FREE_RESPONSE_STREAMS or stream_id_key in ZULIP_FREE_RESPONSE_STREAMS)
...
def send_reply(client: zulip.Client, source_msg: dict, content: str) -> None:
    ...
    for chunk in chunks:
        ...
        if result.get("result") != "success" and is_stream_message(source_msg) and "topic" in request:
...
def main() -> None:
    ...
    print(f"Connected to Zulip as {bot_full_name} <{ZULIP_BOT_EMAIL}> user_id={bot_user_id}")
    ...
    def handle_message(msg: dict) -> None:
        try:
            ...
            where = (
                f"{get_stream_name(msg)} / {get_topic(msg)}" if is_stream_message(msg) else f"DM from {sender_email}"
            )
