...
def get_client() -> zulip.Client:
    if not ZULIP_SITE_URL or not ZULIP_BOT_EMAIL or not ZULIP_API_KEY:
        raise RuntimeError(
            "Missing ZULIP_SITE_URL, ZULIP_BOT_EMAIL, or ZULIP_API_KEY in .env"
        )
...
def download_attachment(candidate: AttachmentCandidate, target_dir: Path) -> Path:
    ...
    with requests.get(signed_url, stream=True, timeout=60) as response:
        ...
        if content_length and int(content_length) > max_bytes:
            raise ValueError(
                f"Attachment too large: {int(content_length)} bytes > {max_bytes} bytes"
            )
        ...
        with output_path.open("wb") as f:
            for chunk in response.iter_content(chunk_size=1024 * 1024):
                ...
                if total > max_bytes:
                    raise ValueError(
                        f"Attachment exceeded max size while downloading: {MAX_DOWNLOAD_MB} MB"
                    )
...
def ocr_pdf_file(path: Path) -> str:
    ...
    if len(doc) > total_pages:
        lines.append(
            f"\n[OCR truncated: processed {total_pages}/{len(doc)} pages. "
            f"Increase ZULIP_OCR_MAX_PDF_PAGES if needed.]"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="zulip-attachments-") as tmp:
        ...
        for index, (msg, candidate) in enumerate(items[:max_files], start=1):
            ...
            if msg:
                ...
                header.append(
                    f"- sender: {msg.get('sender_full_name') or msg.get('sender_email')}"
                )
...
def zulip_search_messages(
    ...
    if used_client_scan:
        payload["note"] = "Results include a client-side content scan because Zulip full-text search can lag or miss long bot replies."
    if not formatted:
...
def zulip_extract_attachment_url(
    ...
    if not safe_attachment_url(full_url):
        return (
            "Refusing to download this URL. "
            "Only this Zulip server's /user_uploads/ URLs are allowed."
        )
